var OysterApp = {
  initMap: function () {
    ...
    map = L.map('map').setView([40.68, -97.63], 4);
    ...
    function addPopups() {
      markers.eachLayer(function (layer) {
        ...
        var ptid = pt.id;
        var content =
          '<div class="popup"><div class="projectname"><h2>' +
          pt.noaaname +
          ', ' +
          pt.state +
          '</h2></div>' +
          ...
          pt.id +
          "'" +
          'class="btn btn-primary map-load-data" data-toggle="modal" data-target="#map-modal">Use this site</button>' +
          '</div>';
    ...
    map.on('popupopen', function (e) {
      ...
      $('#map').on('click', '#' + markerInfo.id, function () {
        if (ractive.get('siteType') === 'existing') {
          ...
          ractive.set(
            'activeSite.bayresidencetime_d_historic',
            markerInfo['bayresidencetime_d']
          );
          ractive.set(
            'activeSite.tsummeravg_historic',
            markerInfo['tsummeravg']
          );
          ...
          ractive.set(
            'activeSite.length2massequation',
            markerInfo.length2massequation
          );
          ractive.set(
            'activeSite.length2massrefname',
            markerInfo.length2massrefname
          );
          ractive.set(
            'activeSite.length2massreferencefull',
            markerInfo.length2massreferencefull
          );
          ractive.set('activeSite.ecoregion', markerInfo.ecoregion);
          ractive.set(
            'activeSite.tsummeravg',
            Math.round(markerInfo.tsummeravg)
          );
  ...
  format: function (num) {
    ...
    if (num > 1000)
      return Math.floor(num / 1000) + ',' + (num % 1000).toFixed(3);
    return num;
  },
  ...
  calculateFiltrationRate: function (
    oysterType,
    oysterDensity,
    length,
    length2RefName,
  ...
  init: function () {
    ...
    $('#map-modal').on('shown.bs.modal', function () {
      // chooseLocation is the id of the modal.
      //map.invalidateSize();
      setTimeout(function () {
        map.invalidateSize();
      }, 50);
    ...
    ractive = new MyRactive({
      ...
      computed: {
        bayFilterVol: function () {
          ...
          bayVol =
            estuaryVolLiters / this.get('activeSite.bayresidencetime_d') / 24;
        ...
        bayFilterVolHistoric: function () {
          ...
          bayVol =
            estuaryVolLiters /
            this.get('activeSite.bayresidencetime_d_historic') /
            24;
          ractive.set('activeSite.bayfiltervolhistoric', bayVol);
          return bayVol;
        },
        oysterWeight: function () {
          var estuaryVolLiters =
            this.get('activeSite.bayvolume_1000m3') * 1000000; // xls units are in m3 * 1000 1m3 = 1000 L
          return (
            estuaryVolLiters / this.get('activeSite.bayresidencetime_d') / 24
          );
        },
        historicFiltration: function () {
          ...
          if (this.get('activeSite.historicmeanlength_mm')) {
            filtration =
              this.get('activeSite.historicreefarea_ha') *
              10000 *
              OysterApp.calculateFiltrationRate(
                this.get('activeSite.oystertype'),
                this.get('activeSite.historictotaldensity_ind_m2'),
                this.get('activeSite.historicmeanlength_mm'),
                this.get('activeSite.length2massrefname'),
                this.get('activeSite.tsummeravg_historic')
              );
            ...
            filtration =
              ...
              (OysterApp.calculateFiltrationRate(
                this.get('activeSite.oystertype'),
                this.get('activeSite.historicculldensity_ind_m2'),
                this.get('activeSite.historiccullmeanlength_mm'),
                this.get('activeSite.length2massrefname'),
                this.get('activeSite.tsummeravg_historic')
              ) +
                OysterApp.calculateFiltrationRate(
                  this.get('activeSite.oystertype'),
                  this.get('activeSite.historicmarketdensity_ind_m2'),
                  this.get('activeSite.historicmarketmeanlength_mm'),
                  this.get('activeSite.length2massrefname'),
                  this.get('activeSite.tsummeravg_historic')
                ));
            ractive.set('activeSite.historicfiltration', filtration);
        ...
        currentFiltration: function () {
          ...
          if (
            this.get('activeSite.currentculldensity_ind_m2') !== null ||
            this.get('activeSite.currentmarketdensity_ind_m2') !== null ||
            this.get('goalData.goalmarketdensity') ||
            this.get('goalData.goalculldensity')
          ) {
            if (this.get('activeSite.currentculldensity_ind_m2') === null) {
              console.log('null, dude');
            }
            filtration =
              this.get('activeSite.currentreefarea_ha') *
              10000 *
              (OysterApp.calculateFiltrationRate(
                this.get('activeSite.oystertype'),
                this.get('activeSite.currentculldensity_ind_m2'),
                this.get('activeSite.currentcullmeanlength_mm'),
                this.get('activeSite.length2massrefname'),
                this.get('activeSite.tsummeravg')
              ) +
                OysterApp.calculateFiltrationRate(
                  this.get('activeSite.oystertype'),
                  this.get('activeSite.currentmarketdensity_ind_m2'),
                  this.get('activeSite.currentmarketmeanlength_mm'),
                  this.get('activeSite.length2massrefname'),
                  this.get('activeSite.tsummeravg')
                ));
            ...
            goalOneOysterCull = OysterApp.calculateOneOysterFilt(
              this.get('activeSite.oystertype'),
              this.get('goalData.goalculllength'),
              this.get('activeSite.length2massrefname'),
              this.get('activeSite.tsummeravg')
            );
            goalOneOysterMarket = OysterApp.calculateOneOysterFilt(
              this.get('activeSite.oystertype'),
              this.get('goalData.goalmarketlength'),
              this.get('activeSite.length2massrefname'),
              this.get('activeSite.tsummeravg')
            );
            ractive.set('goalData.goalculloneoysterrate', goalOneOysterCull);
            ractive.set(
              'goalData.goalmarketoneoysterrate',
              goalOneOysterMarket
            );
            filtrationGoal =
              this.get('activeSite.bayfiltervol') *
                (this.get('goalData.goalfiltrationprcnt') / 100) -
              this.get('activeSite.currentfiltration');
            ...
            filtration =
              ...
              OysterApp.calculateFiltrationRate(
                this.get('activeSite.oystertype'),
                this.get('activeSite.currenttotaldensity_ind_m2'),
                this.get('activeSite.currentmeanlength_mm'),
                this.get('activeSite.length2massrefname'),
                this.get('activeSite.tsummeravg')
              );
            ractive.set('activeSite.currentfiltration', filtration);
        ...
        sumFishIndiv: function () {
          ...
          for (var key in fishArray) {
            if (fishArray[key].abundanceWeight === 0) {
              ...
              var indivSum =
                (fishArray[key].juvMeanHa * goalFiltrationHa) /
                (10000 *
                  (this.get('goalData.goalmarketdensity') *
                    this.get('goalData.goalmarketoneoysterrate') +
                    this.get('goalData.goalculldensity') *
                      this.get('goalData.goalculloneoysterrate')));
            } else if (fishArray[key].abundanceWeight === 2) {
              var indivSum =
                ((fishArray[key].juvMeanHa +
                  fishArray[key].juvStDevAcre / 0.404686) *
                  goalFiltrationHa) /
                (10000 *
                  (this.get('goalData.goalmarketdensity') *
                    this.get('goalData.goalmarketoneoysterrate') +
        ...
        sumFishBiomass: function () {
          ...
          for (var key in fishArray) {
            if (fishArray[key].abundanceWeight === 0) {
              ...
              var indivSum =
                (fishArray[key].meanProductionKgHa * goalFiltrationHa) /
                (10000 *
                  (this.get('goalData.goalmarketdensity') *
                    this.get('goalData.goalmarketoneoysterrate') +
                    this.get('goalData.goalculldensity') *
              ...
              var indivSum =
                ((fishArray[key].meanProductionKgHa +
                  fishArray[key].stDevProductionKgAcre / 0.404686) *
                  goalFiltrationHa) /
                (10000 *
                  (this.get('goalData.goalmarketdensity') *
                    this.get('goalData.goalmarketoneoysterrate') +
